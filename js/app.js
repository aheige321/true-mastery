...
const syncService = {
    ...
    init() {
        // 每30秒检查一次云端
        setInterval(() => this._pollCloud(), 30000);
        // 标记轮询中
        this.isPolling = true;
    },
    ...
    scheduleSync(delay = 5000) {
        ...
        this._debounceTimer = setTimeout(() => this.syncData(), delay);
    },
    ...
    async syncData() {
        ...
        this.lastSyncTime = result.gistUpdatedAt;
    ...
    async forceDownload() {
        const response = await fetch(this.endpoint, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ method: 'OVERWRITE_LOCAL' })
        });
...
// 数据存储模块 - 添加 save 时自动触发同步
const _originalSave = store.save;
store.save = function() {
    _originalSave.call(this);
    // 自动同步（有防抖）
